...
import {
  getSessionDoNotShowThisAgain,
  setSessionDoNotShowThisAgain
} from "../../../utils/common";
...
const ParkingProcessInScreen = ({ location, history }) => {
  ...
  const [
    ...
    setNoResponseFromParkingDevice
  ] = useState(false);

  const [
    doorClosedBeforeDetectorFires,
    setDoorClosedBeforeDetectorFires
  ] = useState(false);
  ...
  useEffect(() => {
    openBoxTimeout.current = setTimeout(function () {
      BoxDataService.get(boxId).then(data => {
        if (
          data.data.state === NEITHER_PARKING_NOT_RENTING ||
    ...
    <>
          ...
          <Col>
            {doorClosedBeforeDetectorFires ? (
              <MyMarker
                ...
                )}. ${t("Click continue to start parking again...")}.`}
                icon={faInfoCircle}
              />
            ) : stateParkingProcess ===
              PARKING_MODE_INTRODUCING_SCOOTER_ORDER_TO_OPEN_DOOR_SENT ? (
              ...
              <MyMarker
                ...
                state={null}
                text={t(
                  "The door is opened. Please, introduce your scooter and plug the charger in."
                )}
                icon={faInfoCircle}
              />
            ) : stateParkingProcess ===
              ...
              <>
                <MyMarker
                  ...
                  state={null}
                  text={t(
                    "The door is closed. The parking process is complete."
                  )}
                  icon={faInfoCircle}
                />
              </>
      ...
      >
        ...
        <DialogContent dividers={true}>
          ...
          >
            {t(
              "Press on the logo at the right left corner of the door to open it."
            )}
